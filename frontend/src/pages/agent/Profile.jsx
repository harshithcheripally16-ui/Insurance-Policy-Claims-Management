...
const Profile = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const updated = await agentService.updateProfile(updateData);
      setProfile(updated);
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} md={4}>
          <Card><CardContent sx={{ textAlign: 'center', py: 4 }}>
            <Box sx={{ width: 80, height: 80, borderRadius: '50%', bgcolor: '#eff6ff', display: 'flex', alignItems: 'center', justifyContent: 'center', mx: 'auto', mb: 2, border: '2px solid #bfdbfe' }}>
              <PersonIcon sx={{ fontSize: 40, color: '#1d4ed8' }} />
            </Box>
            <Typography variant="h6" fontWeight="700">{profile?.full_name}</Typography>
            <Typography variant="body2" color="text.secondary">{profile?.email}</Typography>
            <Divider sx={{ my: 2 }} />
            <Box sx={{ display: 'flex', justifyContent: 'space-around' }}>
              <Box><Typography variant="h6" fontWeight="700" color="primary.main">{profile?.purchases_count}</Typography><Typography variant="caption" color="text.secondary">Purchases</Typography></Box>
            </Box>
          </CardContent></Card>
        </Grid>
          ...
          <Card><CardContent>
            ...
            <form onSubmit={handleSubmit}>
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} sx={{ textAlign: 'right' }}><Button type="submit" variant="contained" disabled={saving}>{saving ? 'Saving...' : 'Save Changes'}</Button></Grid>
              </Grid>
            </form>
          </CardContent></Card>
        </Grid>
